...
class BotSelfTest:
    ...
    def setup_test_environment(self):
        ...
        if 'resources.db' in self.test_db or self.test_db == 'resources.db':
            error = 'SAFETY ERROR: Test database path conflicts with production!'
        ...
        if actual_db_path != self.test_db:
            error = f'SAFETY ERROR: Bot is using {actual_db_path} instead of test DB {self.test_db}'
            raise RuntimeError(error)
    ...
    def test_database_initialization(self):
        ...
        for column in required_columns:
            self.assert_test(
                ...
                'Required column found in items table',
            )
    ...
    def test_moderator_management(self):
        ...
        self.assert_test(
            not duplicate,
            'Reject duplicate moderator',
            'Duplicate moderator correctly rejected',
        )
        ...
        self.assert_test(
            len(moderators) == 2,
            'List moderators count',
            f'Found {len(moderators)} moderators',
        )
    ...
    def test_authorized_users(self):
        ...
        self.assert_test(
            not duplicate,
            'Reject duplicate authorized user',
    ...
    def test_notification_system(self):
        ...
        self.assert_test(
            not duplicate,
            'Reject duplicate notification',
            'Duplicate notification correctly rejected',
        )
        ...
        self.assert_test(
            len(chats) >= 1,
            'Get notification chats for type',
            f'Found {len(chats)} chats for type',
        )
