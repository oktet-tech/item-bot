...
class BotSelfTest:
    ...
    def setup_test_environment(self):
        ...
        print("🔒 Production database (resources.db) is completely safe and untouched")
    ...
    def assert_test(self, condition: bool, test_name: str, details: str = ""):
        ...
        self.test_results.append({"name": test_name, "status": status, "details": details, "passed": condition})
    ...
    def test_database_initialization(self):
        ...
        for table in required_tables:
            self.assert_test(table in tables, f"Table '{table}' exists", "Required table found in database")
        ...
        for column in required_columns:
            self.assert_test(
                column in columns, f"Items table has '{column}' column", "Required column found in items table"
            )
    ...
    def test_item_management(self):
        ...
        success1 = self.retry_db_operation(
            lambda: self.bot.add_item("WebServer1", "production", server_type_id, "Main web server")
        )
        self.assert_test(success1, "Add item 'WebServer1'", "Item added successfully")

        success2 = self.retry_db_operation(
            lambda: self.bot.add_item("iPhone15", "testing", device_type_id, "Test device")
        )
        self.assert_test(success2, "Add item 'iPhone15'", "Item added successfully")

        # Test duplicate item rejection
        duplicate = self.retry_db_operation(
            lambda: self.bot.add_item("WebServer1", "production", server_type_id, "Duplicate")
        )
        ...
        self.assert_test(found_by_id == webserver_id, "Find item by ID", "Found same item by ID")

        # Test item editing (description)
        success, message = self.retry_db_operation(
            lambda: self.bot.edit_item_description(webserver_id, "Updated web server description")
        )
    ...
    def test_note_system(self):
        ...
        items = self.bot.list_items()
        webserver_item = next(i for i in items if i["name"] == "WebServer1")
        self.assert_test(webserver_item["note"] is None, "Verify note removal", "Note correctly removed from database")
    ...
    def test_item_operations(self):
        ...
        items = self.bot.list_items()
        webserver_item = next(i for i in items if i["name"] == "WebServer1")
        self.assert_test(webserver_item["owner"] == "alice", "Verify item ownership", "Item correctly assigned to user")
        self.assert_test(
            webserver_item["purpose"] == "debugging issue", "Verify item purpose", "Purpose correctly stored"
        )
        ...
        webserver_item = next(i for i in items if i["name"] == "WebServer1")
        self.assert_test(
            webserver_item["owner"] == "bob", "Verify stolen item ownership", "Item ownership correctly transferred"
        )
        ...
        items = self.bot.list_items()
        webserver_item = next(i for i in items if i["name"] == "WebServer1")
        self.assert_test(webserver_item["owner"] is None, "Verify item is free", "Item correctly freed")
    ...
    def test_notification_system(self):
        ...
        success = self.bot.add_notification(
            chat_id=-123456, chat_title="Test Chat", type_id=server_type_id, added_by="admin"
        )
        ...
        duplicate = self.bot.add_notification(
            chat_id=-123456, chat_title="Test Chat", type_id=server_type_id, added_by="admin"
        )
    ...
    def test_usage_history(self):
        ...
        self.assert_test(
            history_count >= 2, "Usage history recorded", f"Found {history_count} history entries for item operations"
        )
    ...
    def test_item_filtering(self):
        ...
        self.assert_test(
            len(production_items) >= 1, "Filter items by group", f"Found {len(production_items)} production items"
        )
        ...
        self.assert_test(len(server_items) >= 1, "Filter items by type", f"Found {len(server_items)} server items")

        # Test filter by owner (should be none since we freed everything)
        owned_items = self.bot.list_items(owner="testuser")
        self.assert_test(
            len(owned_items) == 0, "Filter items by owner", f"Found {len(owned_items)} items owned by testuser"
        )

        # Test free items only
        free_items = self.bot.list_items(free_only=True)
        self.assert_test(len(free_items) >= 2, "Filter free items only", f"Found {len(free_items)} free items")
